...
void main (void)
{
    ...
    UART0_ini_38400_24MHz();      // 38400 Baud rate for DELTA
...
END_2:
          ...
          if (uart_rcvbuf[1]==0xE3 && uart_rcvbuf[2]==0xD2 && uart_rcvbuf[3]==0xC1)
          {
             if  (g_connectflag==1)
              {
...
END_1:
              ...
              if (g_connectflag==0)
              { 
                if (uart_rcvbuf[0]==CMD_CONNECT)
                  {
                    g_connectflag=1;

                    Package_checksum();
                    Send_64byte_To_UART0();
                    g_timer0Counter=50; //5000; //1unit = 4m sec ////reset time out counter
                    g_timer0Over=0;
                  }
              }
            }
